...
import { apiHandler } from "../entities/ApiError";
...
export const updateUser = apiHandler(async (req: any, res: any, next: any) => {
...
export const getUser = apiHandler(async (req: any, res: any, next: any) => {
...
export const login = apiHandler(async (req: any, res: any, next: any) => {
  // This id is as key used user
  const body = req.body;
  let objUser: object = {};
...
export const getAllUser = apiHandler(async (req: any, res: any, next: any) => {
  // Command to get user data from firebase
...
export const addNewUser = apiHandler(async (req: any, res: any, next: any) => {
  // Command to get user data from firebase
  ...
  const queries = query(
    collection(db, "user-data"),
    or(where("email", "==", body.email), where("name", "in", [`${body.name}`]))
  );
